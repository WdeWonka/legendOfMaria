...
const Cutscenes = (() => {
  ...
  async function playOpening(callback) {
    ...
    await showCutsceneText('"Los sueños se ganan."\n"Ven a encontrarnos."', 2500);

    // Phase 5: Friend enters
    await drawFriendScene(1000);
    await showCutsceneText("Sofía: ¡María!\n¡Esto es terrible!", 1800);
    await showCutsceneText("Sofía: ¡Tu traje desapareció!\n¡Y tu bastón también!", 2000);
    await showCutsceneText("Sofía: ¡El campeonato empieza pronto!\n¡DEBES encontrarlos!", 2200);
  ...
  function drawSleepingScene(duration) {
    return new Promise(resolve => {
      ...
      function frame() {
        ...
        ctx.beginPath(); ctx.arc(380, 30, 18, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = '#e8e8a0';
        ctx.beginPath(); ctx.arc(378, 28, 14, 0, Math.PI*2); ctx.fill();
        // Stars
        ctx.fillStyle = '#ffffff';
        [[300,20],[340,10],[420,30],[450,15],[390,55]].forEach(([sx,sy]) => {
        ...
        ctx.beginPath();
        ctx.moveTo(320, 40); ctx.lineTo(420, 40);
        ctx.lineTo(350, 320); ctx.lineTo(250, 320);
        ctx.fill();
        ctx.restore();
  ...
  function drawStarfield() {
    ...
    function frame() {
      ...
      for (let i = 0; i < 60; i++) {
        ...
        finalCtx.fillRect(Math.round(sx), Math.round(sy), 1 + (i % 2), 1 + (i % 2));
      }
  ...
  function drawVictoryScene(duration) {
    return new Promise(resolve => {
      ...
      const confetti = Array.from({length:40}, (_, i) => ({
        x: Math.random()*480, y: -20 - Math.random()*100,
        vx: Utils.randFloat(-20,20), vy: Utils.randFloat(40,80),
      ...
      function frame() {
        ...
        if (t > 0.5) {
          ...
          ctx.beginPath(); ctx.arc(295, 180, 20, Math.PI, 0); ctx.fill();
  ...
  function drawConfetti(duration) {
    return new Promise(resolve => {
      ...
      const pieces = Array.from({length:60}, (_, i) => ({
        x: Math.random()*480, y: -30 - Math.random()*100,
        vx: Utils.randFloat(-40,40), vy: Utils.randFloat(60,120),
        color: ['#ff80c0','#d4a0ff','#ffe040','#ff4488','#80ffff','#ffaa00'][i%6],
      ...
      function frame() {
        ...
        for (const p of pieces) {
          ...
          if (p.y > 340) { p.y = -15; p.x = Math.random()*480; }
          ctx.save();
